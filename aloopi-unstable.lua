...
local function _typeof(o)
    ...
    if t == 'table' then
        success, meta = pcall(function() return getmetatable(o) end)
        if success then
        ...
        local function recurse(dim, lvl)
            ...
            if dim_size > 0 then
                if n_of_items_in(dim) ~= dim_size then
                    -- error 
                    if size >= 0 then
                        error('Array does not contain ' .. (size > 0 and size or 'any') .. ' items');
                    end
                end
            end
            ...
            for k, v in next, dim do
                ...
                if lvl < num_of_dims then
                    ...
                    for i, v in next, dim do
                        if _typeof(v) ~= container.dt and _typeof(v) ~= 'nil' then
                            -- check datatypes of items
                            error('Element in dimension \'' .. lvl .. '\' at index \'' .. i .. '\' is not of datatype \'' .. container.dt .. '\'')
                        end
                    end
                end
            end
...
local function check_dictionary(container, arg)
    ...
    if _typeof(arg) == 'table' then
        for k, v in next, arg do
            if _typeof(k) ~= kdt and not k_optional then
                -- key error
                error('Keys\' datatype is \'' .. _typeof(k) .. '\', not datatype \'' .. kdt .. '\' in \'' .. container.raw_key ..'\'')
            end

            if _typeof(v) ~= vdt and not v_optional then
                -- value error
...
local function check_type(container, arg)
    ...
    elseif at ~= dt and dt ~= 'any' then
        if at == 'nil' and not container.optional then
            ...
            while not found ')' do
                if found 'ID' or found '[' then
                    ...
                    if found ',' then
                        consume ','
                    elseif found 'ID' then
                        error('Was expecting \')\' not \'' .. token.lexme .. '\'')
                    end
                elseif found ',' then
                    -- for 'any' shorthand
                    consume ','
...
function BaseClass.new(members, super_class)
    ...
    local class = setmetatable({
        ...
        __newindex = function(self, key, val)
            ...
            if obj_t == 'Container' then
                -- set container value
                self._static_members.public[key] = set_container_value(obj, val)
            else
                -- set value
        ...
        __call = function(self, ...)
            if not meta.__init then error('__init is not defined in class \'' .. type ..'\'') end
